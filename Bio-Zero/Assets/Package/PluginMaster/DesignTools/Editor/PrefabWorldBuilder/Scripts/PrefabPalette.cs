...
namespace PluginMaster
{
    public class PrefabPalette : UnityEditor.EditorWindow, ISerializationCallbackReceiver
    {
        ...
        private void SettingsContextMenu()
        {
            ...
            menu.AddItem(new GUIContent("Cleanup"), false, () => 
            { 
        ...
        private void TabBar()
        {
            ...
            using (new GUILayout.HorizontalScope(UnityEditor.EditorStyles.toolbar))
            {
                ...
                for (int i = 0; i <= this.lastVisibleIdx; ++i)
                {
                    ...
                    {
                        var menu = new UnityEditor.GenericMenu();
                        menu.AddItem(new GUIContent("Rename"), false, this.ShowRenamePaletteWindow,
                            new RenameData(i, name, position.position + Event.current.mousePosition));
                        menu.AddItem(new GUIContent("Delete"), false, this.ShowDeleteConfirmation, i);
                        menu.ShowAsContext();
                    }
                }
                GUILayout.FlexibleSpace();
